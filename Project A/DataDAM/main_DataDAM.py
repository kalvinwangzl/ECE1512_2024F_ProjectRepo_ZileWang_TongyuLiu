...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='MNIST', help='dataset: can use MNIST, CIFAR10 or MHIST')
    parser.add_argument('--model', type=str, default='ConvNet', help='model')
    parser.add_argument('--ipc', type=int, default=40, help='image(s) per class')
    parser.add_argument('--eval_mode', type=str, default='SSS', help='eval_mode') 
    ...
    parser.add_argument('--batch_real', type=int, default=16, help='batch size for real data')
    ...
    parser.add_argument('--save_path', type=str, default='./save_data', help='path to save results')
    ...
    # print(np.arange(0, args.Iteration+1, args.Iteration+1))
    # exit()
    ...
    class MHISTDataset(Dataset):
        ...
        def __getitem__(self, idx):
            img_name = os.path.join(self.images_dir, self.annotations.iloc[idx, 0])  # 获取图片文件名
            image = Image.open(img_name).convert("RGB")  # 打开图片并转换为RGB模式
            label = self.annotations.iloc[idx, -1]  # 获取标签
    ...
    if args.dataset == 'MHIST':
        ...
        # 数据路径
        data_path = './mhist_dataset'  # 数据集的路径
        images_dir = os.path.join(data_path, 'images')  # 图片文件夹路径
        annotations_file = os.path.join(data_path, 'annotations.csv')  # 注释文件路径
        ...
        bs = 64
        dataloader = DataLoader(all_dataset, batch_size=bs, shuffle=False)

        # 初始化累积变量
        mean = torch.zeros(3)
        std = torch.zeros(3)

        # 遍历数据集，累积每个通道的像素值和平方值
    ...
    if args.dataset != 'MHIST':
        print(args.dataset)
        channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader, zca = get_dataset(args.dataset, args.data_path, args)
    # model_eval_pool = get_eval_pool(args.eval_mode, args.model, 'ConvNetD2')
    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                ...
                save_image(image_syn_vis, save_name, nrow=20) # Trying normalize = True/False may get better visual effects.
